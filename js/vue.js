...
function factory() {
     ...
     * 创建一个Map, 后续用于判断某个 key 在不在这个 Map 中
     */
     ...
     * 将中划线连接改为首字母大写, 比如 main-container 改为 mainContainer, 该转换结果会进行缓存
     */
    var camelizeRE = /-(\w)/g;
        ...
        if (computed) {
            for (var key in computed) {
                ...
                if (typeof userDef === 'function') {
                    ...
                    if (userDef.get) {
                        if (userDef.cache !== false) {
                            computedSharedDefinition = makeComputedGetter(userDef.get, vm);
                        } else {
                            computedSharedDefinition = bind$1(userDef.get, vm);
                        }
                    } else {
                        noop();
                    }
                    if (userDef.set) {
                        computedSharedDefinition.set = bind$1(userDef.set, vm);
                    } else {
                        computedSharedDefinition.set = noop;
                    }
                }
